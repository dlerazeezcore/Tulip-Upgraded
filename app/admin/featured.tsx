          ...
          <View style={{ backgroundColor: t.bgElev, borderRadius: 14, borderColor: t.border, borderWidth: 1, overflow: 'hidden' }}>
            {vm.rows.map((r, i) => (
              <View key={r.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, paddingHorizontal: 14, borderBottomWidth: i === vm.rows.length - 1 ? 0 : 1, borderBottomColor: t.border, opacity: (r.enabled ?? true) ? 1 : 0.5 }}>
                ...
                <Pressable onPress={() => vm.toggleEnabled(r)} accessibilityRole="button" accessibilityLabel={(r.enabled ?? true) ? tr('admin.featured.hideFromFeatured') : tr('admin.featured.showInFeatured')} accessibilityState={{ selected: (r.enabled ?? true) }} hitSlop={8} style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: t.bgSunken, alignItems: 'center', justifyContent: 'center' }}>
                  {(r.enabled ?? true) ? <Eye size={15} color={t.fg} /> : <EyeOff size={15} color={t.fgMuted} />}
                </Pressable>
                <Pressable onPress={() => vm.onDelete(r)} accessibilityRole="button" accessibilityLabel={tr('a11y.delete')} hitSlop={8} style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: t.dangerBg, alignItems: 'center', justifyContent: 'center' }}>
