...
import { ChevronLeft, Send, Check, Copy } from 'lucide-react-native';
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: t.bg }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40, gap: 16, maxWidth: 640, width: '100%', alignSelf: 'center' }}>
        {/* Audience picker */}
        <Text style={{ fontSize: 11, fontWeight: '700', color: t.fgMuted, textTransform: 'uppercase' }}>Audience</Text>
        <View style={{ gap: 8 }}>
        ...
        <Text style={{ fontSize: 11, color: t.fgFaint, marginTop: -8 }}>
          English is required (used as fallback). Arabic and Kurdish are optional.
        </Text>
        ...
        <PrimaryButton label={vm.sending ? 'Sending…' : 'Send'} onPress={vm.send} />
      </ScrollView>
    </SafeAreaView>
  );
}
