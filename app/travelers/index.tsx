...
import { TravelerListSkeleton } from '@/components/Skeleton';
    ...
    <ScreenSafeArea style={{ flex: 1, backgroundColor: t.bg }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 20, paddingVertical: 12 }}>
        ...
        <Pressable
          ...
          accessibilityRole="button"
          hitSlop={8}
          style={{ flexDirection: 'row', alignItems: 'center', gap: 5, paddingVertical: 8, paddingHorizontal: 14, borderRadius: t.radius.pill, backgroundColor: t.primary }}
        >
       ...
       <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginHorizontal: isWide ? -t.gridGutterHalf : 0, gap: isWide ? 0 : 10 }}>
        {vm.firstLoading ? (
          ...
          <View style={{ width: '100%' }}>
            <EmptyState
              icon={User}
              title={tt('travelers.noTravelersTitle')}
              subtitle={tt('travelers.noTravelersSub')}
              action={<PrimaryButton label={tt('travelers.addTraveler')} icon={<Plus size={15} color={t.onPrimary} strokeWidth={2.4} />} onPress={openNew} />}
            />
          </View>
        ) : (
          travelers.map((tr) => (
            <View key={tr.id} style={{ width: isWide ? '50%' : '100%', padding: isWide ? t.gridGutterHalf : 0 }}>
            <View
              style={{
                flexDirection: 'row',
                alignItems: 'center',
      ...
      {/* Add / edit modal */}
      <BottomSheet visible={editing !== null} onClose={() => setEditing(null)} gap={14} avoidKeyboard>
            ...
            {vm.saveError ? (
              <Text style={{ fontSize: 12, color: t.danger, textAlign: 'center' }}>{vm.saveError}</Text>
            ) : null}
